...
项目根目录/
├── .specflow/
│   ├── SPECFLOW.md                     # 单一事实来源（合并 AGENTS/CONSTITUTION/RULES）
│   ├── docs/
...
- **文档即中枢**：`SPECFLOW.md` / `SPEC.md` / `ACCEPTANCE.md` 是唯一事实来源
...
- **跨平台验收协议**：通过 Review Packet + 状态机 + 人类确认契约实现闭环，不依赖平台专有 UI
...
- `/specflow`：总入口（支持 `init`、`feature <name>`、`align`、`accept <name>`）
...
- 人类确认契约：`/specflow approve <feature-name>` 或 `/specflow reject <feature-name>`
- 产出 `COMPLETION_REPORT.md`
- 归档 active feature 并更新 memory

### 跨平台 Review Packet（推荐）

为获得类似 Plan 的稳定体验（不依赖平台自动弹窗），每个待验收 feature 维护：
- `.specflow/reviews/<feature>/REVIEW.md`（人类评审入口）
- `.specflow/reviews/<feature>/STATUS.json`（机器可读状态）
...
说明：
- “可点击文件列表”是导航层；`STATUS.json` 才是流程层与恢复依据。
