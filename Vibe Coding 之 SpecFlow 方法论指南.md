...
- **需求澄清/设计阶段**：`docs/superpowers/specs/**` 为主，但必须受 `.specflow/SPECFLOW.md` / `.specflow/docs/*` 的约束
...
- 测试用例生成后，可手动修改以强化边界。
- 验收报告中任何不符，可打回重做。
- 技术选型变更时，手动修改 `.specflow/SPECFLOW.md`。
...
| **技术变更** | `“我修改了 .specflow/SPECFLOW.md 中的 [某部分]，请先产出对齐回执并按需对齐相关代码/测试，不做不必要全量重扫。”` |
...
- `SPECFLOW_TEMPLATE.md`（单一事实来源模板：合并 AGENTS/CONSTITUTION/RULES）
